...
server.registerTool(
  ...
  async ({ workspace, project_id, state, priority, label }) => {
    ...
    if (state) params.set('state__group', state)
    if (priority) params.set('priority', priority)
    if (label) params.set('label__name', label)
    const query = params.toString() ? `?${params}` : ''
    const data = await planeGet(
      `/api/v1/workspaces/${workspace}/projects/${project_id}/issues/${query}`
    )
    const issues: any[] = data.results ?? data
...
server.registerTool(
  ...
  async ({ workspace, project_id }) => {
    ...
    const result = {
      ...
      states: (states.results ?? states).map((s: any) => ({ id: s.id, name: s.name, group: s.group })),
      members: (members.results ?? members).map((m: any) => ({
        id: m.member ?? m.id,
...
server.registerTool(
  ...
  async ({ workspace, project_id, issue_id }) => {
    ...
    const comments: any[] = data.results ?? data
...
server.registerTool(
  ...
  async ({ workspace, query }) => {
    const data = await planeGet(
      `/api/v1/workspaces/${workspace}/issues/?search=${encodeURIComponent(query)}`
    )
    const issues: any[] = data.results ?? data
    const summary = issues.map((i: any) => ({
      id: i.id,
