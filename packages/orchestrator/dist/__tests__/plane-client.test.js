...
describe('PlaneClient', () => {
    ...
    it('updates issue state', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('http://plane-api:8000/api/v1/workspaces/ws/projects/proj-1/issues/issue-1/', expect.objectContaining({ method: 'PATCH', body: JSON.stringify({ state: 'state-2' }) }));
    });
    it('posts a comment', async () => {
        mockFetch.mockResolvedValueOnce({ ok: true, json: async () => ({ id: 'c1' }) });
        await client.addComment('ws', 'proj-1', 'issue-1', 'Agent started');
        expect(mockFetch).toHaveBeenCalledWith('http://plane-api:8000/api/v1/workspaces/ws/projects/proj-1/issues/issue-1/comments/', expect.objectContaining({ method: 'POST' }));
    });
    ...
    it('lists workspace members', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('http://plane-api:8000/api/v1/workspaces/my-workspace/members/', expect.objectContaining({ headers: expect.objectContaining({ 'X-API-Key': 'test-token' }) }));
    ...
    it('registers a webhook', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('http://plane-api:8000/api/v1/workspaces/my-workspace/webhooks/', expect.objectContaining({ method: 'POST', body: expect.stringContaining('hook.io') }));
    ...
    it('fetches issue comments', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('http://plane-api:8000/api/v1/workspaces/ws/projects/proj-1/issues/issue-1/comments/', expect.objectContaining({ headers: expect.objectContaining({ 'X-API-Key': 'test-token' }) }));
